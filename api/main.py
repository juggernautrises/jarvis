...
def jarvis_instakill():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('instakill'):
            ...
            db.collection('jarvis').document('ironjug_state').update(
                {"instakill": instakill,
                 'date_updated': datetime.datetime.now()})
            d = db.collection('jarvis').document(
                'ironjug_state').get().to_dict()
            return jsonify({
                'success': True,
...
def jarvis_open():
    ...
    if request.method == 'POST':
        if request.form.get('open'):
            ...
            d = db.collection('jarvis').document('ironjug_state').update(
                {"open": open_status, 'date_updated': datetime.datetime.now()})
            d = db.collection('jarvis').document(
                'ironjug_state').get().to_dict()
            return jsonify({
                'success': True,
